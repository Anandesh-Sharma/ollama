...
const defaultQwen35TimingLogEvery = 64

type qwen35TimingReporter struct {
	once sync.Once
	...
	modelCalls       int64
	modelPrefillCall int64
	modelDecodeCall  int64
	modelTokens      int64
	modelTotalNS     int64
	embedNS          int64
	normNS           int64
	...
	fullAttnCalls       int64
	fullAttnPrefillCall int64
	fullAttnDecodeCall  int64
	fullAttnTotalNS     int64

	gdnCalls       int64
	gdnPrefillCall int64
	gdnDecodeCall  int64
	gdnTotalNS     int64
	gdnConvNS      int64
	gdnCoreNS      int64
	...
	moeCalls         int64
	moeQuantCalls    int64
	moeFallbackCalls int64
	moeSortCalls     int64
}

var qwen35TimingStats qwen35TimingReporter

func qwen35RunnerLogf(format string, args ...any) {
	_, _ = fmt.Fprintf(os.Stderr, "qwen3.5 timing: "+format+"\n", args...)
}

// qwen35Env* helpers are used for qwen3.5-specific runtime overrides and
// diagnostics. Performance defaults are encoded in code and these env vars are
// intended as local escape hatches / profiling toggles, not a user-facing API.
func qwen35EnvBoolNames(def bool, names ...string) bool {
...
func qwen35EnvBool(name string, def bool) bool {
	return qwen35EnvBoolNames(def, name)
}

func qwen35EnvPositiveIntNames(def int, names ...string) int {
	for _, name := range names {
...
func qwen35EnvPositiveInt(name string, def int) int {
	return qwen35EnvPositiveIntNames(def, name)
}

func qwen35Timing() *qwen35TimingReporter {
	qwen35TimingStats.once.Do(func() {
		// Prefer generic model timing env names, but keep qwen3.5-specific aliases
		// for compatibility with existing local perf workflows.
		qwen35TimingStats.enabled = qwen35EnvBoolNames(false,
			"OLLAMA_MLX_MODEL_TIMING",
			"OLLAMA_MLX_QWEN3_5_TIMING",
		)
		qwen35TimingStats.logEvery = int64(qwen35EnvPositiveIntNames(defaultQwen35TimingLogEvery,
			"OLLAMA_MLX_MODEL_TIMING_EVERY",
			"OLLAMA_MLX_QWEN3_5_TIMING_EVERY",
		))
		if qwen35TimingStats.enabled {
			qwen35RunnerLogf("enabled every=%d", qwen35TimingStats.logEvery)
		}
	})
	if !qwen35TimingStats.enabled {
		return nil
	}
	return &qwen35TimingStats
}

func (t *qwen35TimingReporter) ensureLayersLocked(n int) {
	if n <= len(t.layerTotalNS) {
		return
	}
	grow := n - len(t.layerTotalNS)
	t.layerTotalNS = append(t.layerTotalNS, make([]int64, grow)...)
	t.layerCalls = append(t.layerCalls, make([]int64, grow)...)
	t.layerLinear = append(t.layerLinear, make([]bool, grow)...)
}

func (t *qwen35TimingReporter) recordLayer(i int, isLinear bool, d time.Duration) {
	t.mu.Lock()
	t.ensureLayersLocked(i + 1)
	t.layerTotalNS[i] += d.Nanoseconds()
	t.layerCalls[i]++
	t.layerLinear[i] = isLinear
	t.mu.Unlock()
}

func (t *qwen35TimingReporter) recordFullAttention(L int32, d time.Duration) {
	t.mu.Lock()
	t.fullAttnCalls++
	if L > 1 {
		t.fullAttnPrefillCall++
	} else {
		t.fullAttnDecodeCall++
	}
	t.fullAttnTotalNS += d.Nanoseconds()
	t.mu.Unlock()
}

func (t *qwen35TimingReporter) recordGatedDelta(
	L int32,
	total, conv, core time.Duration,
	conv1d, setConv, split, prep, post, setDelta time.Duration,
) {
	t.mu.Lock()
	t.gdnCalls++
	if L > 1 {
	...
	t.gdnTotalNS += total.Nanoseconds()
	t.gdnConvNS += conv.Nanoseconds()
	t.gdnCoreNS += core.Nanoseconds()
	t.gdnConv1DNS += conv1d.Nanoseconds()
	t.gdnSetConvNS += setConv.Nanoseconds()
	t.gdnSplitNS += split.Nanoseconds()
	t.gdnPrepNS += prep.Nanoseconds()
	t.gdnPostNS += post.Nanoseconds()
	t.gdnSetDeltaNS += setDelta.Nanoseconds()
	t.mu.Unlock()
}

func (t *qwen35TimingReporter) recordGatedDeltaKernelHit(hit bool) {
	t.mu.Lock()
	if hit {
		t.gdnKernelHit++
	} else {
...
func (t *qwen35TimingReporter) recordMoEPath(useQuantized, didSort bool) {
	t.mu.Lock()
	t.moeCalls++
	if useQuantized {
		t.moeQuantCalls++
...
func (t *qwen35TimingReporter) topLayersLocked(limit int) string {
	type entry struct {
		idx   int
		total int64
		calls int64
	}
	best := make([]entry, 0, limit)
	for i := range t.layerTotalNS {
		if t.layerCalls[i] == 0 {
			continue
		}
		e := entry{idx: i, total: t.layerTotalNS[i], calls: t.layerCalls[i]}
		inserted := false
		for j := range best {
			if e.total > best[j].total {
	...
	for i, e := range best {
		...
		kind := "full"
		if e.idx < len(t.layerLinear) && t.layerLinear[e.idx] {
			kind = "linear"
		}
		avgMs := float64(e.total) / float64(e.calls) / float64(time.Millisecond)
		out += fmt.Sprintf("L%d(%s)=%.2fms", e.idx, kind, avgMs)
	}
	return out
}

func (t *qwen35TimingReporter) recordModelForward(L int32, total, embed, norm time.Duration) {
	var line string
	...
	t.modelTokens += int64(L)
	t.modelTotalNS += total.Nanoseconds()
	t.embedNS += embed.Nanoseconds()
	t.normNS += norm.Nanoseconds()

	shouldLog := t.logEvery > 0 && t.modelCalls%t.logEvery == 0
	if shouldLog {
		modelAvgMs := float64(t.modelTotalNS) / float64(t.modelCalls) / float64(time.Millisecond)
		avgTokens := float64(t.modelTokens) / float64(t.modelCalls)

		var fullAttnAvgMs float64
		if t.fullAttnCalls > 0 {
			fullAttnAvgMs = float64(t.fullAttnTotalNS) / float64(t.fullAttnCalls) / float64(time.Millisecond)
		}

		var gdnAvgMs, gdnConvPct, gdnCorePct float64
		var gdnConv1dPct, gdnSetConvPct, gdnSplitPct, gdnPrepPct, gdnPostPct, gdnSetDeltaPct float64
		if t.gdnCalls > 0 {
			gdnAvgMs = float64(t.gdnTotalNS) / float64(t.gdnCalls) / float64(time.Millisecond)
			if t.gdnTotalNS > 0 {
				gdnConvPct = 100 * float64(t.gdnConvNS) / float64(t.gdnTotalNS)
				gdnCorePct = 100 * float64(t.gdnCoreNS) / float64(t.gdnTotalNS)
				gdnConv1dPct = 100 * float64(t.gdnConv1DNS) / float64(t.gdnTotalNS)
				gdnSetConvPct = 100 * float64(t.gdnSetConvNS) / float64(t.gdnTotalNS)
				gdnSplitPct = 100 * float64(t.gdnSplitNS) / float64(t.gdnTotalNS)
				gdnPrepPct = 100 * float64(t.gdnPrepNS) / float64(t.gdnTotalNS)
				gdnPostPct = 100 * float64(t.gdnPostNS) / float64(t.gdnTotalNS)
				gdnSetDeltaPct = 100 * float64(t.gdnSetDeltaNS) / float64(t.gdnTotalNS)
			}
		}
		...
		normPct := 0.0
		if t.modelTotalNS > 0 {
			embedPct = 100 * float64(t.embedNS) / float64(t.modelTotalNS)
			normPct = 100 * float64(t.normNS) / float64(t.modelTotalNS)
		}
		gdnKernelTotal := t.gdnKernelHit + t.gdnKernelMiss
		gdnKernelHitPct := 0.0
		if gdnKernelTotal > 0 {
			gdnKernelHitPct = 100 * float64(t.gdnKernelHit) / float64(gdnKernelTotal)
		}
		moeQuantPct := 0.0
		moeSortPct := 0.0
		if t.moeCalls > 0 {
			moeQuantPct = 100 * float64(t.moeQuantCalls) / float64(t.moeCalls)
			moeSortPct = 100 * float64(t.moeSortCalls) / float64(t.moeCalls)
		}

		line = fmt.Sprintf(
			"calls=%d decode=%d prefill=%d avg_model_ms=%.2f avg_tokens=%.2f embed_pct=%.1f norm_pct=%.1f "+
				"full_attn_avg_ms=%.2f full_attn_decode=%d full_attn_prefill=%d "+
				"gdn_avg_ms=%.2f gdn_conv_pct=%.1f gdn_core_pct=%.1f gdn_decode=%d gdn_prefill=%d "+
				"gdn_conv1d_pct=%.1f gdn_set_conv_pct=%.1f gdn_split_pct=%.1f gdn_prep_pct=%.1f gdn_post_pct=%.1f gdn_set_delta_pct=%.1f "+
				"gdn_kernel_hit=%d gdn_kernel_miss=%d gdn_kernel_hit_pct=%.1f "+
				"moe_calls=%d moe_quant=%d moe_fallback=%d moe_quant_pct=%.1f moe_sort_pct=%.1f "+
				"top_layers=%q",
			t.modelCalls, t.modelDecodeCall, t.modelPrefillCall, modelAvgMs, avgTokens, embedPct, normPct,
			fullAttnAvgMs, t.fullAttnDecodeCall, t.fullAttnPrefillCall,
			gdnAvgMs, gdnConvPct, gdnCorePct, t.gdnDecodeCall, t.gdnPrefillCall,
			gdnConv1dPct, gdnSetConvPct, gdnSplitPct, gdnPrepPct, gdnPostPct, gdnSetDeltaPct,
			t.gdnKernelHit, t.gdnKernelMiss, gdnKernelHitPct,
			t.moeCalls, t.moeQuantCalls, t.moeFallbackCalls, moeQuantPct, moeSortPct,
			t.topLayersLocked(4),
		)
	}
	t.mu.Unlock()
...
func (m *Model) LoadWeights(tensors map[string]*mlx.Array) error {
	...
	if qwen35Timing() != nil && len(moeLoadSummaries) > 0 {
		qwen35RunnerLogf("moe load summary count=%d", len(moeLoadSummaries))
		for _, line := range moeLoadSummaries {
			qwen35RunnerLogf("%s", line)
		}
	}
...
func (a *FullAttention) Forward(x *mlx.Array, c cache.Cache, B, L int32, cfg *Config) *mlx.Array {
	...
	if tr != nil {
		tr.recordFullAttention(L, time.Since(t0))
	}
	return out
}
...
func (g *GatedDeltaNet) Forward(x *mlx.Array, c cache.Cache, B, L int32, cfg *Config) *mlx.Array {
	tr := qwen35Timing()
	var t0, tPhase time.Time
	if tr != nil {
		t0 = time.Now()
	}
	var convDur, coreDur time.Duration
	var conv1dDur, setConvDur, splitDur, prepDur, postDur, setDeltaDur time.Duration
	...
	fastRecurrentWrite := L == 1 && qwen35FastRecurrentWrite()
	...
	if tr != nil {
		splitDur = time.Since(tPhase)
		convDur = time.Since(tPhase)
	}
	...
	if tr != nil {
		prepDur = time.Since(tPhase)
		convDur = conv1dDur + setConvDur + splitDur + prepDur
	}
	...
	if fusedOut, fusedState, fused := mlx.GatedDeltaKernel(q, k, v, gDecay, beta, state); fused {
		if tr != nil {
			tr.recordGatedDeltaKernelHit(true)
		}
		out = fusedOut
		state = fusedState
	} else if L == 1 {
		if tr != nil {
			tr.recordGatedDeltaKernelHit(false)
		}
		if repeatFactor > 1 {
	...
	if rc != nil {
		...
		rc.Advance(int(L))
		if tr != nil {
			setDeltaDur = time.Since(tPhase)
		}
	}
	if tr != nil {
		tr.recordGatedDelta(L, time.Since(t0), convDur, coreDur, conv1dDur, setConvDur, splitDur, prepDur, postDur, setDeltaDur)
	}
	return out
}
...
func (s *SwitchMLP) Forward(x *mlx.Array, indices *mlx.Array, cfg *Config) *mlx.Array {
	...
	doSort := B*L >= 64
	if tr != nil {
		tr.recordMoEPath(s.UseQuantized, doSort)
	}
...
func (m *Model) Forward(tokens *mlx.Array, caches []cache.Cache) *mlx.Array {
	...
	for i, layer := range m.Layers {
		...
		if tr != nil {
			tr.recordLayer(i, layer.IsLinear, time.Since(tStep))
		}
	}
	if tr != nil {
		tStep = time.Now()
	...
	if tr != nil {
		normDur := time.Since(tStep)
		tr.recordModelForward(L, time.Since(t0), embedDur, normDur)
	}
	return out
}
...
// LowMemoryDecode defaults to the async decode pipeline for throughput. Set
// OLLAMA_MLX_QWEN3_5_LOW_MEMORY_DECODE=1 to force the low-memory decode path on
// memory-constrained systems.
func (m *Model) LowMemoryDecode() bool {
	return qwen35EnvBool("OLLAMA_MLX_QWEN3_5_LOW_MEMORY_DECODE", false)
}

// Fast recurrent cache writes are enabled by default for qwen3.5 decode because
// per-layer snapshot/eval dominates recurrent layer time. Set
// OLLAMA_MLX_QWEN3_5_FAST_RECURRENT_WRITE=0 to force the safer materialized path
// for debugging/regression checks.
func qwen35FastRecurrentWrite() bool {
	return qwen35EnvBool("OLLAMA_MLX_QWEN3_5_FAST_RECURRENT_WRITE", true)
}

// EnableCompile defaults to MLX compile for better decode throughput. Set
// OLLAMA_MLX_QWEN3_5_ENABLE_COMPILE=0 to disable for debugging/regression
// checks.
func (m *Model) EnableCompile() bool {
	return qwen35EnvBool("OLLAMA_MLX_QWEN3_5_ENABLE_COMPILE", true)
}
